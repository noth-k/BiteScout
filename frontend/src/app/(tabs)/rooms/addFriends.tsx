...
const addFriendsScreen = () => {
  ...
  const { previousScreen, currUsers, roomId } = route.params as {
  ...
  useEffect(() => {
    const fetchUsers = async () => {
      try {
        ...
        setUsers(
          initialUsers.filter((user: User) => user._id != authUser?._id)
        );
      } catch (error) {
  ...
  const handleSubmitUsers = () => {
    ...
    currSelectedUsers = removeCommonObjects(
      currSelectedUsers,
      selectedUsersState
    );
  ...
  const handleAddUsers = async () => {
    ...
    const newUserIds = selectedUsers
      .map((user) => user?._id || "")
      .filter((userId) => !currUsers.includes(userId));
    console.log(newUserIds);

    const newUserRestrictions = selectedUsers
      .filter((user) => !currUsers.includes(user?._id || ""))
      .map((user) => user.restrictions);
  ...
  return (
    <View style={{ backgroundColor: "white", height: "100%" }}>
      ...
      <Text
        style={styles.submit}
        onPress={
          previousScreen == "Create" ? handleSubmitUsers : handleAddUsers
        }
      >
        Let's go!
      </Text>
